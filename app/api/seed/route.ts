...
const FOOTBALL_TEAMS: Record<string, string[]> = {
  ...
  laliga: ['Реал Мадрид', 'Барселона', 'Атлетико Мадрид', 'Жирона', 'Атлетик Бильбао', 'Бетис'],
  bundesliga: ['Байер Л.', 'Бавария', 'Штутгарт', 'РБ Лейпциг', 'Боруссия Д', 'Айнтрахт'],
  seriea: ['Интер', 'Милан', 'Ювентус', 'Аталанта', 'Болонья', 'Рома'],
  ucl: ['Реал Мадрид', 'Манчестер Сити', 'Бавария', 'ПСЖ', 'Интер', 'Барселона'],
};
...
type MarketType = '1X2' | 'TOTAL' | 'HANDICAP' | 'BOTH_SCORE' | 'DOUBLE_CHANCE';

interface MarketTemplate {
  names: string[];
  generateOdds: () => { name: string; odds: number }[];
}

const MARKET_TEMPLATES: Record<MarketType, MarketTemplate> = {
  '1X2': {
    names: ['Исход матча', 'Результат матча (1X2)', 'Победитель'],
    generateOdds: () => {
      const margin = 0.04 + Math.random() * 0.03;
      const p1 = 0.3 + Math.random() * 0.4;
      const pDraw = 0.2 + Math.random() * 0.15;
      const p2 = 1 - p1 - pDraw;
...
export async function POST() {
  try {
    ...
    for (const sport of WINLINE_SPORTS) {
      ...
      // Генерируем события для футбола
      if (sport.id === 'football') {
        for (const tournament of FOOTBALL_TOURNAMENTS) {
          ...
          const teams = FOOTBALL_TEAMS[tournament.id] || FOOTBALL_TEAMS.rpl;
          const shuffled = shuffleArray(teams);

          for (let i = 0; i < Math.min(3, shuffled.length / 2); i++) {
      ...
      if (sport.id === 'hockey') {
        ...
        for (let i = 0; i < 4; i++) {
          ...
          await sql`
            INSERT INTO events (external_id, sport_id, tournament_id, name, home_team, away_team, start_time, is_live, last_updated)
            VALUES (${eventId}, ${sportDbId}, ${tournamentDbId}, ${`${homeTeam} - ${awayTeam}`}, ${homeTeam}, ${awayTeam}, ${getRandomDate(5)}, false, NOW())
            ON CONFLICT (external_id) DO UPDATE SET last_updated = NOW()
          `;
          totalEvents++;
        }
...
export async function GET() {
  return NextResponse.json({
    ...
    usage: 'curl -X POST https://uno-eosin.vercel.app/api/seed',
  });
}

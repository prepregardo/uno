...
export class WinlineBrowser {
  ...
  async login(): Promise<boolean> {
    ...
    if (!login || !password) {
      console.warn('⚠️ WINLINE_LOGIN or WINLINE_PASSWORD not set, skipping auth');
      return false;
    }
    ...
    try {
      ...
      // Ищем кнопку входа
      const loginButton = await this.page.$('[data-test="login-button"], .login-btn, button:has-text("Вход")');
      ...
      if (loginButton) {
        ...
        const loginInput = await this.page.$('input[name="login"], input[type="tel"], input[placeholder*="Телефон"]');
        ...
        if (loginInput && passwordInput) {
          ...
          if (submitButton) {
            ...
            const userMenu = await this.page.$('[data-test="user-menu"], .user-balance, .account-balance');
            if (userMenu) {
      ...
      console.warn('⚠️ Could not complete login flow');
      return false;
    } catch (error) {
